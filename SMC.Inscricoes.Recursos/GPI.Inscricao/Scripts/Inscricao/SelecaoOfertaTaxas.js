...
$(document).ready(function () {
    var divInstrucoes = $('#divInstrucoesTaxa'),
        ...
        numeroMaximoOferta = smc.core.uicontrol('NumeroMaximoOfertaPorInscricao').getValue(),
        ofertas = $('#Ofertas'),
        permiteMultiplaOferta = numeroMaximoOferta == '' || parseInt(numeroMaximoOferta) > 1,
        boletoPago = smc.core.uicontrol("PossuiBoletoPago").getValue() == "True";

    var displayAdditionalInfo = function (element, loading) {
        var obj = smc.core.uicontrol(element),
            val = obj.getValue();

        // Se o processo possuir boleto pago, permitir mais de uma oferta ou se a tela estiver carregando, não deverá atualizar a taxa se já existir alguma preenchida.
        if ((boletoPago || permiteMultiplaOferta || loading) && divListaTaxas.html().trim() != '')
            return;
    ...
    $("button.smc-lookup-open-command").on("click", function () {
        setTimeout(() => {
            $("div.smc-alerta-processando").css('display', 'none');
        }, 200);        
    });
